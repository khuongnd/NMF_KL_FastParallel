function [W, F, his] = nmf_kl(V, r, opts, varargin)
% function [W,H,errs,vout] = nmf_kl(V,r,varargin)
%
...
[niter, thresh, norm_w, norm_h, verb, myeps, W0, H0, W, H] = ...
    parse_opt(varargin, 'niter', 100, 'thresh', [], 'norm_w', 0, ...
...
for t = 1:niter,
    ...
    disp(['nmf_kl: iter= ' num2str(t) ' time = ' num2str(current) ' , err=' num2str(errs(t))]);
end

% display error if asked
if verb >= 2
    disp(['nmf_kl: final_err=' num2str(errs(t))]);
end
