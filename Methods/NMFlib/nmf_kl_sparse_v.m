function [W, F, his] = nmf_kl_sparse_v(V, r, opts, varargin)
% function [W,H,errs,vout] = nmf_kl_sparse_v(V,r,varargin)
%
% Implements NMF using the normalized Kullback-Leibler divergence (see [1]
% for details) with the sparsity constraints proposed in [2]:
%
%      min D(V||W*H) + alpha*phi(H) s.t. W>=0, H>=0
%
%   where phi(H) is a sparsity penalty defined in [2]
%
...
%                   can be 0 (none), 1 (1-norm), 2 (2-norm), 'a' (sum(H(:))=1)
...
%     NIPS, 2001
% [2] Virtanen, T.  Monaural Sound Source Separation by Non-Negative
%     Factorization with Temporal Continuity and Sparseness Criteria, IEEE
%     Transactions on Audio, Speech, and Language Processing, vol. 15(3), 2007.
%
...
% process arguments
[niter, thresh, norm_w, norm_h, verb, myeps, W0, H0, W, H, alpha] = ...
...
for t = 1:niter,
    ...
    if update_H
        % update H
        denom = W'*Onm + alpha*((1/m * H.^2*Omm).^(-1/2));
        H = H .* ( (W'*(V./R) + alpha*((H.*(sqrt(m)*H*Omm)) ./ ...
                                       ((H.^2*Omm).^(3/2)))) ./ ...
                   max(denom, myeps) );
        if norm_h ~= 0
            H = normalize_H(H,norm_h);
    ...
    s_errs(t) = sum(abs(H(:)./sigma(:)));
    errs(t) = I_errs(t) + alpha*s_errs(t);
    times(t) = toc(start) + current;
    current = times(t);
    
    % display error if asked
    %if verb >= 3
    fprintf(1, ['nmf_kl_sparse_v: iter=%d, I-div=%f, sparse_err=%f (alpha=%f), ' ...
...
% display error if asked
if verb >= 2
   fprintf(1, ['nmf_kl_sparse_v: final, I-div=%f, sparse_err=%f (alpha=%f), ' ...
